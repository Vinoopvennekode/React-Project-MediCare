...
function Profile({setProfile}) {
  ...
  useEffect(() => {
    axios
      .post(`/profile`, { data: id }, { headers: { Authorization: token } })
      .then((res) => {
...
<div className=" p-10 bg-gray-100 rounded-lg overflow-auto shadow-xl transform transition-all opacity-90 fixed inset-0 z-50  ">
      <div className=" flex h-screen justify-center items-center ">
        <div className="flex-col justify-center  bg-gray-200 py-12 px-10 ">
        ...
        <button onClick={()=>setProfile(false)} className="bg-gray-300 p-2">back</button>
        </div>
      </div>
      </div>
      </div>
